...
bool StartLayer::init()
{
    ...
    //3D飞机的创建
    palne3D = Sprite3D::create("startPic/zdj3D.obj");
    palne3D->setTexture("startPic/HE162R.jpg");
    palne3D->setPosition(Point(220,400));
    palne3D->setScale(0.015f);
    palne3D->setRotation3D(Vertex3F(30,45,0));
   	this->addChild(palne3D,10);

    plane3D = Sprite3D::create("startPic/plane.obj");
    plane3D->setTexture("startPic/F111tail.jpg");
	...
	//背景精灵的创建
    Sprite* StartBgSp = Sprite::create("startPic/startBg.png");
    StartBgSp->setPosition(Point(visibleSize.width/2,visibleSize.height/2));
    this->addChild(StartBgSp,0);

    //标题的创建
    Sprite* StartTitleSp = Sprite::create("startPic/main_title.png");
	...
	for(int i = 0 ;i<10;i++)
	{
		Sprite* xzTiaoSp = Sprite::create("startPic/xzTiao.png");
	...
	if(ChooseLevelLayer::bacMusicFlag == true)
	{
		CocosDenshion::SimpleAudioEngine::getInstance()//播放背景音乐
					->playBackgroundMusic("mysound/sound_bgm_title.mp3",true);
	}
	//创建一个单点触摸监听
...
void StartLayer::aboutCallback()
{
	...
	//创建标题
	aboutTTSp = Sprite::create("startPic/taskTitle.png");
	...
	//关于主背景
	Sprite* aboutBgSp = Sprite::create("startPic/aboutBg.png");
	aboutBgSp->setAnchorPoint(Point(0,1));
	aboutBgSp->setPosition(Point(0,0));
	aboutTTSp->addChild(aboutBgSp,10);

	//3D球的创建
	earth3D = Sprite3D::create("startPic/earth.obj");
	earth3D->setScale(5.0f);
	earth3D->setTexture("startPic/worldmap.png");
...
void StartLayer::helpCallback()
{
	...
	//创建标题
	aboutTTSp = Sprite::create("startPic/helpTT.png");
	aboutTTSp->setPosition(Point(260,500));
	...
	//创建帮助主背景
	Sprite* helpBgSp = Sprite::create("startPic/helpBg.png");
	helpBgSp->setAnchorPoint(Point(0,1));
	helpBgSp->setPosition(Point(-3,0));
	aboutTTSp->addChild(helpBgSp,10);

	//创建3D模型
	helpCube = Sprite3D::create("help/help.obj");
	helpCube->setTexture("help/Texture.png");
	...
	for(int i=0;i<6;i++)
	{
		if(i == 0)
		{
			bwSp[i] = Sprite::create("help/blank.png");
		}else
		{
			bwSp[i] = Sprite::create("help/white.png");
		}
...
void StartLayer::firstEnter()
{
	...
	{
		Sprite* tsSp = Sprite::create("help/finger.png");
...
void StartLayer::initSound()
{
	CocosDenshion::SimpleAudioEngine::getInstance()							//背景音效
					->preloadBackgroundMusic("mysound/sound_bgm_title.mp3");

	CocosDenshion::SimpleAudioEngine::getInstance()							//即使音效
					->preloadEffect("mysound/sound_sfx_click.mp3");
}
//设置菜单项回调的方法
void StartLayer::selBTCallback()
{
	...
	bselItem = MenuItemImage::create(
			"startPic/startButtonSel.png",
			"startPic/startButtonSelp.png",
	...
	sdsLabel = LabelAtlas::create(				//创建一个LabelAtlas对象
			StringUtils::format("%d",sdsSt),"gamePic/labelatlas.png",15,19,'0');
	...
	Sprite* djSp = Sprite::create("startPic/stara.png");
	...
	moneyLabel = LabelAtlas::create(				//创建一个LabelAtlas对象
			StringUtils::format("%d",moneySt),"gamePic/labelatlas.png",15,19,'0');
...
void StartLayer::clearCallback()
{
	...
	aboutItem->setEnabled(false);
	helpItem->setEnabled(false);
	czcdSp = Sprite::create("startPic/czcdTT.png");
	czcdSp->setPosition(Point(480,400));
	this->addChild(czcdSp,3);

	Sprite* bbSp = Sprite::create("startPic/bb.png");
...
void StartLayer::onTouchEnded(Touch* touch, Event* event)
{
	...
	{
		spHelp = Sprite::create(StringUtils::format("help/help%d.png",helpIndex));
