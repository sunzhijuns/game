...
bool ChooseLevelLayer::init()
{
	...
	worldMapSp = Sprite::create("chooseLevelPic/worldmapc.png");
	...
	levelCount = levelNum;
	std::string levelString = "chooseLevelPic/xsjx.png";
	if(levelNum == 1)
	{
		levelString = "chooseLevelPic/nextLevel.png";
	}
	...
	{
		//指定线路路径
		std::string linePath = StringUtils::format("chooseLevelPic/line%d.png",i);
		//创建线路
	...
	if(levelNum == 1)
	{
		Sprite* kqzdSp = Sprite::create("chooseLevelPic/kqzd.png");
	...
	Sprite* maoDianSp= Sprite::create("chooseLevelPic/maodian.png");
	...
	for(int i=0;i<2;i++)
	{
		Sprite* tempSp = Sprite::create(StringUtils::format("chooseLevelPic/jqs%d.png",i));
		...
		LabelAtlas* tempLabel = LabelAtlas::create(				//创建一个LabelAtlas对象
				StringUtils::format("%d",tempCount),"gamePic/labelatlas.png",15,19,'0');
		if(i==0)
		{
	...
	if(ChooseLevelLayer::bacMusicFlag == true)
	{
		CocosDenshion::SimpleAudioEngine::getInstance()//播放背景音乐
							->playBackgroundMusic("mysound/sound_bgm_worldmap.mp3",true);
	}
...
{
	std::string linePath = StringUtils::format("chooseLevelPic/line%d.png",levelCount-1);
	Sprite* lineSp = Sprite::create(linePath);
	if(levelCount==2)
	{
		lineSp->setAnchorPoint(Point(1,0));
...
void ChooseLevelLayer::initMenuItem()
{
	...
	menuItem[0] = MenuItemImage::create(
			"chooseLevelPic/menuset.png",
			"chooseLevelPic/menusetp.png",
			CC_CALLBACK_0(ChooseLevelLayer::szCallback, this)
	);
	menuItem[1] = MenuItemImage::create(
			"chooseLevelPic/menusj.png",
			"chooseLevelPic/menusjp.png",
			CC_CALLBACK_0(ChooseLevelLayer::sjCallback, this)
	);
	menuItem[2] = MenuItemImage::create(
			"chooseLevelPic/menucj.png",
			"chooseLevelPic/menucjp.png",
	...
	for(int i=0;i<3;i++)
	{
		...
		Sprite* menuBgSp = Sprite::create("chooseLevelPic/menuBg.png");
		std::string slabel = StringUtils::format("chooseLevelPic/label%d.png",i);
...
void ChooseLevelLayer::bacMusicMenuCallback()
{
	...
	{
		sfN = SpriteFrame::create("chooseLevelPic/bacMusicOffc.png",Rect(0,0,33,35));
		sfS = SpriteFrame::create("chooseLevelPic/bacMusicOnc.png",Rect(0,0,33,35));
		CocosDenshion::SimpleAudioEngine::getInstance()->pauseBackgroundMusic();
		UserDefault::getInstance()->setIntegerForKey(Constant::BGSY.c_str(),1);
	}else
	{
		sfN = SpriteFrame::create("chooseLevelPic/bacMusicOnc.png",Rect(0,0,33,35));
		sfS = SpriteFrame::create("chooseLevelPic/bacMusicOffc.png",Rect(0,0,33,35));
...
void ChooseLevelLayer::froMusicMenuCallback()
{
	...
	{
		sfN = SpriteFrame::create("chooseLevelPic/froMusicOffc.png",Rect(0,0,39,30));
		sfS = SpriteFrame::create("chooseLevelPic/froMusicOnc.png",Rect(0,0,39,30));
		UserDefault::getInstance()->setIntegerForKey(Constant::YSSY.c_str(),1);
	}else
	{
		sfN = SpriteFrame::create("chooseLevelPic/froMusicOnc.png",Rect(0,0,39,30));
		sfS = SpriteFrame::create("chooseLevelPic/froMusicOffc.png",Rect(0,0,39,30));
...
void ChooseLevelLayer::szCallback()
{
	...
	musicItem[0] = MenuItemImage::create(
			"chooseLevelPic/bacMusicOnc.png",
			"chooseLevelPic/bacMusicOffc.png",
	...
	musicItem[1] = MenuItemImage::create(
			"chooseLevelPic/froMusicOnc.png",
			"chooseLevelPic/froMusicOffc.png",
...
void ChooseLevelLayer::cjCallback()
{
	...
	setMenuEnabled(false);
	selecItem = true;
	taskSp = Sprite::create("chooseLevelPic/cjtaskTitle.png");
	...
	Sprite* aboutBgSp = Sprite::create("chooseLevelPic/cjbg.png");
	aboutBgSp->setAnchorPoint(Point(0,1));
	aboutBgSp->setPosition(Point(0,0));
	taskSp->addChild(aboutBgSp,10);

	ship3DSp = Sprite3D::create("chooseLevelPic/ship3D.obj");
	ship3DSp->setTexture("chooseLevelPic/shipWL.png");
	ship3DSp->setPosition(Point(330,130));
	ship3DSp->setScale(2.8f);
	aboutBgSp->addChild(ship3DSp,100);

	MenuItemImage* closeItem = MenuItemImage::create(
			"chooseLevelPic/close.png",
			"chooseLevelPic/closep.png",
	...
	for(int i =0;i<4;i++)
	{
		...
		countLabel[i] = LabelAtlas::create(				//创建一个LabelAtlas对象
				countString,"gamePic/labelatlas.png",15,19,'0');
...
void ChooseLevelLayer::initSound()
{
	CocosDenshion::SimpleAudioEngine::getInstance()					//背景音效
						->preloadBackgroundMusic("mysound/sound_bgm_worldmap.mp3");
}
...
void ChooseLevelLayer::PopTaskWindow(int levelIndex)
{
	...
	taskSp = Sprite::create("chooseLevelPic/taskBg.png");
	taskSp->setPosition(Point(visibleSize.width/2-100,visibleSize.height/7*4));
	this->addChild(taskSp,1);

	std::string taskPath = StringUtils::format("chooseLevelPic/level%d.png",levelIndex);
	...
	//地图上爆炸=======================end
	Sprite* taskTitleSp = Sprite::create("chooseLevelPic/taskTitle.png");
	...
	for(int i=0;i<2;i++)
	{
		Sprite* bgSp = Sprite::create("chooseLevelPic/bgc.png");
	...
	if(jxs == 1)
	{
		jxcySp = Sprite::create("chooseLevelPic/jx01.png");
	}else
	{
		jxcySp = Sprite::create("chooseLevelPic/jx00.png");
	}
	...
	if(jxg == 1)
	{
		jxcySp = Sprite::create("chooseLevelPic/jx11.png");
	}else
	{
		jxcySp = Sprite::create("chooseLevelPic/jx10.png");
	}
	jxcySp->setAnchorPoint(Point(0,0));
	...
	Label* titleLabel = Label::createWithTTF(labelPath,"fonts/FZKATJW.ttf",30);
	...
	MenuItemImage* ksItem = MenuItemImage::create(
			"chooseLevelPic/button.png",
			"chooseLevelPic/buttonp.png",
	...
	std::string wzbgPath = StringUtils::format("chooseLevelPic/wzbg%d.png",levelIndex);
	...
	Label* ksLabel = Label::createWithTTF("\u5f00\u59cb","fonts/FZKATJW.ttf",30);
	ksLabel->setPosition(Point(85,30));
	ksItem->addChild(ksLabel,0);

	Sprite* miaSp = Sprite::create("chooseLevelPic/mia.png");
	miaSp->setPosition(Point(30,30));
	ksItem->addChild(miaSp,0);

	jbLabel = Label::createWithTTF("\u7b80\u5355","fonts/FZKATJW.ttf",30);
	jbLabel->setPosition(Point(85,30));
	jbItem->addChild(jbLabel,0);

	Sprite* jxSp = Sprite::create("chooseLevelPic/jxc.png");
...
void ChooseLevelLayer::jbMenuCallback()
{
	if(ChooseLevelLayer::froMusicFlag == true)
	{
		CocosDenshion::SimpleAudioEngine::getInstance()							//即使音效
									->playEffect("mysound/sound_sfx_star_3.mp3");
	}
	jbIndex++;
	jbIndex = jbIndex%3;
	switch(jbIndex)
