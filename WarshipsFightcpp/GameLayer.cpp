...
void GameLayer::initRaning()
{
	...
	{
		psq = ParticleSystemQuad::create("lzxt/snow.plist");//从文件中加载粒子系统

	}else if(ChooseLevelLayer::levelNum == 2)
	{
		psq = ParticleSystemQuad::create("lzxt/raining.plist");//从文件中加载粒子系统
...
void GameLayer::initMiscellaneous()
{
	...
	sdsLabel = LabelAtlas::create(				//创建一个LabelAtlas对象
			sLabel,"gamePic/labelatlas.png",15,19,'0');
...
{
	Sprite* oceanbg0Sp = Sprite::create("map/oceanbg0.png");
	oceanbg0Sp->setPosition(Point(568,384));
	map->addChild(oceanbg0Sp,-1);
	for(int i=0;i<2;i++)
	{
		oceanbg1Sp[i] = Sprite::create("map/oceanbg1.png");
...
void GameLayer::puaseCallback()
{
	...
	gamePausebb = Sprite::create("puase/btmbb.png");
	gamePausebb->setPosition(Point(480,270));
	this->addChild(gamePausebb,10);

	Sprite* gameOverttSp = Sprite::create("puase/pausett.png");
	gameOverttSp->setPosition(Point(480,410));
	gamePausebb->addChild(gameOverttSp,12);

	Sprite* gameOverbbSp = Sprite::create("puase/bbp.png");
...
void GameLayer::gameStartDJS()
{
	...
	Sprite* shipWaveSp = Sprite::create("gamePic/shipwave.png");
	...
	Label* waveIndex = Label::createWithTTF(
					StringUtils::format("%d",waveNum),"fonts/FZKATJW.ttf",60
			);
...
{
	Sprite* ringSp = Sprite::create("hzjl/bigBoom5.png");
...
{
	std::string mapPath =StringUtils::format("map/map_%d.tmx",ChooseLevelLayer::levelNum);
	//加载TMX地图
...
void GameLayer::warShipAddChild(WarShipObject* wsoSprite)
{
	...
	Sprite* compassSprite = Sprite::create("gamePic/compass.png");
	...
	Sprite* bqSprite = Sprite::create("gamePic/bq.png");
...
void GameLayer::addWarShip(Point touchPoint ,int seleteNum)
{
	...
	Sprite* MZSprite = Sprite::create("gamePic/MZ.png");
	...
	WarShipObject* wso = WarShipObject::create(
			StringUtils::format("gamePic/warShip%d.png",seleteNum),Point(1120,368),seleteNum,1);
...
void GameLayer::createFiniteTimeActionByVector(WarShipObject* warShip)
{
	...
	Point lastPoint=warShip->startPoint;
	...
	for(tempPoint = warShip->warShipPath->begin();tempPoint != warShip->warShipPath->end();tempPoint++)
	{
		...
		if(warShip->rotation != tempRotation)
		{
			...
			RotateTo* rtemp = RotateTo::create(moveT,warShip->rotation);
...
void GameLayer::addWrongPrompt(Point locationMap)
{
	...
	Sprite* wrongSp = Sprite::create("gamePic/wrong.png");
...
void GameLayer::planeFlyAtShip()
{
	...
	for(int i =0;i<2;i++)
	{
		plane3D[i] = Sprite3D::create("startPic/zdj3D.obj");
		plane3D[i]->setTexture("startPic/HE162R.jpg");
...
void GameLayer::initHKMJ()
{
	...
	ParticleSystemQuad* psqOne = ParticleSystemQuad::create("lzxt/tji.plist");//从文件中加载粒子系统
	...
	ParticleSystemQuad* psqTwo = ParticleSystemQuad::create("lzxt/tji.plist");//从文件中加载粒子系统
	...
	for(int i =0;i<20;i++)
	{
		if(i%2==0)
		{
			lifeBarSp[i] = Sprite::create("gamePic/lifeBarY.png");
			lifeBarSp[i]->setPosition(Point(0+tempi*8,100));
			playerHKMJ->hkmjSprite->addChild(lifeBarSp[i],0);
		}else if(i%2==1)
		{
			lifeBarSp[i] = Sprite::create("gamePic/lifeBarG.png");
			lifeBarSp[i]->setPosition(Point(0+tempi*8,100));
...
void GameLayer::initJJItem()
{
	...
	for(int i =0 ;i<2;i++)
	{
		Sprite* itemSp = Sprite::create(StringUtils::format("gamePic/wq%d.png",i));
	...
	for(int i =0 ;i<5;i++)
	{
		Sprite* jjSp = Sprite::create(StringUtils::format("gamePic/jj%d.png",i));
		jjSp->setPosition(Point(45,52));
		jjbgSp[i]->addChild(jjSp,2);
		Sprite* jinbiSp = Sprite::create("gamePic/jinbi.png");
	...
	jjbgSp[4]->addChild(jjName4,12);
	 ...
	 selectedSp = Sprite::create("gamePic/selected.png");
	 selectedSp->setVisible(false);
...
{
	Size size = wsoSprite->getContentSize();
	Sprite* DWSprite = Sprite::create("gamePic/dw.png");
	...
	for(enemytemp = enemyShipVCT->begin();enemytemp != enemyShipVCT->end();enemytemp++)
	{
		...
		for(playertemp = playerShipVCT->begin();playertemp != playerShipVCT->end();playertemp++)
		{
			...
			if(distance < R)
			{
				...
				{
					Sprite* bulletSp = Sprite::create("gamePic/weapon2.png");
						...
						if((*playertemp)->junxian % 4 == 0)
						{
							if((*playertemp)->junxian/4 <= 3)
							{
								...
								if((*playertemp)->junxian/4 == 1)
								{
									...
									{
										CocosDenshion::SimpleAudioEngine::getInstance()
														->playEffect("mysound/sound_sfx_star_1.mp3");
									}

									junxianSp = Sprite::create("gamePic/junx1.png");
								}else
								{
									if(ChooseLevelLayer::froMusicFlag == true)
									{
										std::string musicPath = StringUtils::format("mysound/sound_sfx_star_%d.mp3",(*playertemp)->junxian/4);
										CocosDenshion::SimpleAudioEngine::getInstance()
															->playEffect(musicPath.c_str());
									}

									(*playertemp)->removeChildByTag(JUNXIANSPTAG);
									std::string path = StringUtils::format("gamePic/junx%d.png",(*playertemp)->junxian/4);
...
void GameLayer::reduceBlood()
{
	...
	if(playerHKMJ->lifeValue ==10&&playerHKMJ->qhFlag == false)
	{
		...
		ParticleSystemQuad* psq = ParticleSystemQuad::create("lzxt/huoyan.plist");//从文件中加载粒子系统
...
void GameLayer::gameOver()
{
	...
	if(gameResult == false)
	{
		gameOverttSp = Sprite::create("gameover/titlesb.png");
		gameOverttSp->setPosition(Point(480,430));
		this->addChild(gameOverttSp,12);
		menuPathO = "gameover/cxks.png";
		menuPathT = "gameover/cxksp.png";

		gameOverbbSp = Sprite::create("gameover/bbq.png");
		gameOverbbSp->setAnchorPoint(Point(0,1));
		gameOverbbSp->setPosition(Point(0,0));
		gameOverttSp->addChild(gameOverbbSp,9);

		int index = rand()%4;
		Sprite* gameOvertsSp = Sprite::create(StringUtils::format("gamePic/ts%d.png",index));
	...
	}else if(gameResult == true)
	{
		...
		gameOverttSp = Sprite::create("gameover/titlesl.png");
		gameOverttSp->setPosition(Point(480,430));
		this->addChild(gameOverttSp,12);
		menuPathO = "gameover/jx.png";
		menuPathT = "gameover/jxp.png";
		...
		if(ChooseLevelLayer::froMusicFlag == true)
		{
			std::string musicPath = StringUtils::format("mysound/sound_sfx_star_%d.mp3",starnum);
		...
		{
			Sprite* gameOvertrSp = Sprite::create("gameover/star.png");
