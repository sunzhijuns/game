...
bool UpgradeLayer::init()
{
	...
	Sprite* bacSp = Sprite::create("Upgrade/ship_upgrade_bg.png");
	bacSp->setPosition(Point(480,270));
	this->addChild(bacSp,1);

	Label* titleLabel = Label::createWithTTF("\u795e\u79d8\u5723\u5730","fonts/FZKATJW.ttf",30);
	titleLabel->setPosition(Point(480,527));
	this->addChild(titleLabel,2);

	Sprite* moneySp = Sprite::create("coin/jb.png");
	moneySp->setPosition(Point(930,520));
	moneySp->setScale(0.2);
	this->addChild(moneySp,3);

	Sprite* sjjqSp = Sprite::create("Upgrade/sjjq.png");
	...
	nMoneyLabel = LabelAtlas::create(				//创建一个LabelAtlas对象
			needMSt,"gamePic/labelatlas.png",15,19,'0');
	...
	moneyLabel = LabelAtlas::create(				//创建一个LabelAtlas对象
			moneySt,"gamePic/labelatlas.png",15,19,'0');
	...
	for(int i = 0;i<2;i++)
	{
		std::string spath = StringUtils::format("Upgrade/ddjj%d.png",i);
	...
	if(ChooseLevelLayer::froMusicFlag == true)
	{
		CocosDenshion::SimpleAudioEngine::getInstance()							//即使音效
								->preloadEffect("mysound/getCoin.mp3");
	}
...
void UpgradeLayer::initShipMenu()
{
	...
	MenuItemImage* sjMenuItem =MenuItemImage::create(
			"Upgrade/buttomBg.png",
			"Upgrade/buttomBgp.png",
...
void UpgradeLayer::sjwqMenuCallback()
{
	...
	Sprite* tempCoin = Sprite::create("coin/jb.png");
...
void UpgradeLayer::createCoin()
{
	...
	{
		CocosDenshion::SimpleAudioEngine::getInstance()
								->playEffect("mysound/getCoin.mp3");
	}
...
{
	ParticleSystemQuad* psq = ParticleSystemQuad::create("Upgrade/liang.plist");//从文件中加载粒子系统
...
void UpgradeLayer::disWeaponStar(std::string key)
{
	...
	{
		stars3D[i] = Sprite3D::create("Upgrade/gold3D.obj");
		stars3D[i]->setTexture("Upgrade/glod.png");
	...
	if(temp == 4)
	{
		...
		wqSp = Sprite::create("Upgrade/wq.png");
