...
bool FruitLayer::init()
{
	...
	//准备界面
	ready = Sprite::create("pic/readyFruit.png");
	//设置精灵对象的位置
...
void FruitLayer::initColumn2()
{
	...
	//创建SpriteBatchNode对象
	columnUnder2 = Sprite::create("pic/column1.png");
	//创建SpriteBatchNode对象
	columnOn2 = Sprite::create("pic/column2.png");
	//设置锚点
...
void FruitLayer::update_fruit(float delta)
{
	if(gameFlag)
	{
		...
		string part = "pic/fruit" + number + ".png";
...
void FruitLayer::update_boss(float delta)
{
	if(gameFlag)
	{
		...
		bossFlag = true;
		string number = StringUtils::format("%d", i);
		string partBoss = "pic/fruit" + number + ".png";
		...
		slider->loadProgressBarTexture("pic/blood.png");
...
void FruitLayer::check()
{
	...
	for(iter = fruit->begin(); iter != fruit->end();)
	{
		...
		if(check1 || check2 || check3 || check4)
		{
			...
			this->addChild(psq, 1);
			//为粒子系统设置图片
			psq->setTexture( Director::getInstance()->getTextureCache()->addImage("pic/fire.png") );
			//设置粒子系统的位置
...
void FruitLayer::replay()
{
	...
	//----------------------------------------------------------------------------
	Sprite* over = Sprite::create("pic/gameOver.png");
	over->setPosition(Point(270, 1200));
	this->addChild(over, 10);

	overRank = Sprite::create("pic/scoreOver.png");
	...
	Label* newScore = Label::createWithTTF (tempScore, "fonts/FZKATJW.ttf", 50, Size::ZERO, TextHAlignment::LEFT, TextVAlignment::TOP);
	...
	if(i<count)
	{
		Label* ss = Label::createWithTTF (tempScore, "fonts/FZKATJW.ttf", 50, Size::ZERO, TextHAlignment::LEFT, TextVAlignment::TOP);
	...
	{
		Label* ss = Label::createWithTTF (rank->scoreFruit[0], "fonts/FZKATJW.ttf", 50, Size::ZERO, TextHAlignment::LEFT, TextVAlignment::TOP);
		ss->setColor(Color3B::YELLOW);
		ss->enableOutline(Color4B(187, 187, 187, 255),2);
		overRank->addChild(ss);
		ss->setPosition(Point(320, 50));
...
bool FruitLayer::onTouchPause(Touch* touch, Event* event)
{
	...
	{
		stopFlag = true;
		pause->setTexture("button/continue_pause.png");
